...
//connect to firebase newly created project
...
//getInGame() function created connection to 
//firebase with UUID assigned by firebase
...
//This basically place object into firebase with name, message, time, and idNum
...
//This click function take client chat input and push/add it to firebase
...
//Click function allow uses interact with database
...
//Add user chat data with time stamp
//Display chat history with newest chat at bottom
chatData.orderByChild("time").on("child_added", function(snapshot) {
  ...
  //Allow palyer to review chat history with newest chat at bottom
...
currentTurnRef.on("value", function(snapshot) {
  ...
  if (playerNum) {
    ...
    if (currentTurn === 1) {
      ...
      //receive player choice inout and keep track (append) user choice history
...
//Player must login to start chat
function getInGame() {
  ...
  //Whoever login first will be user 1
  if (currentPlayers < 2) {
    if (playerOneExists) {
      playerNum = 2;
    } else {
    ...
    //Assign player number in first come first serve base
    ...
    //Disconnect player from databse and remove user inforamtion
...
//These are function to take players input and determine win or loss
